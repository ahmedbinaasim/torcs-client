...
while not shutdownClient:
    while True:
        ...
        if buf and b'***identified***' in buf:  # Use bytes for comparison
            print('Received: ', buf)
            break
    ...
    while True:
        ...
        if buf and b'***shutdown***' in buf:  # Use bytes for comparison
            d.onShutDown()
            shutdownClient = True
            print('Client Shutdown')
            break
        
        if buf and b'***restart***' in buf:  # Use bytes for comparison
            d.onRestart()
        ...
        if currentStep != arguments.max_steps:
            if buf != None:
                buf = d.drive(buf).encode()  # Ensure the result is encoded to bytes
        else:
            buf = '(meta 1)'.encode()  # Convert to bytes
